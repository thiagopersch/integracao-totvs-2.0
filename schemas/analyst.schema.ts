...
export const createAnalystSchema = z.object({
  ...
  hourlyRate: z.coerce.number().min(0, "Valor/hora inválido").optional(),
  ...
  level: z.coerce.number().int().min(1, "Nível deve ser no mínimo 1").default(1),
...
export const updateAnalystSchema = z.object({
  ...
  level: z.coerce.number().int().min(1, "Nível deve ser no mínimo 1").optional(),
